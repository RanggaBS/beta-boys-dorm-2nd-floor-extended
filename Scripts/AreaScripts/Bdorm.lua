...
function F_MissionComplete(tblEntry)
  return (shared.gSkippedWeedKiller or IsMissionCompleated(tblEntry.missionID))
    or false
end

function F_FactionDefeated()
  if shared.gSkippedWeedKiller then return true end
...
function F_BikeRaceReward()
  return IsMissionCompleated('3_R08_Rich7')
    and IsMissionCompleated('3_R08_Business4')
    and IsMissionCompleated('3_R08_Poor2')
    and IsMissionCompleated('3_R08_School1')
end

function F_ShootingGalleryReward()
  return shared.gSkippedWeedKiller == true
end
...
function F_CreateRoomTrophies()
  ...
  if F_BikeRaceReward() then
    ...
    idObject, bObjectPool =
      CreatePersistentEntity('Mission_BMX', -496.17, 307.446, 32.707, 0, 14)
  ...
  if GetMissionSuccessCount('2_R03_X') >= 4 then
    ...
    idObject, bObjectPool =
      CreatePersistentEntity('Mission_PR', -496.203, 315.721, 32.7491, 0, 14)
    table.insert(tblRoomTrophies, { id = idObject, bPool = bObjectPool })
    totalTrophies = totalTrophies + 1
  end
  ...
  if shared.gSkippedWeedKiller or F_PurchasedCarPoster() then
    ...
    idObject, bObjectPool = CreatePersistentEntity(
      'Mission_CarnTckt01',
      -493.556,
      307.619,
      33.0466,
  ...
  if shared.gSkippedWeedKiller or StatGetAsInt(266) == 25 then
    ...
    idObject, bObjectPool =
      CreatePersistentEntity('BDorm_GnomeA', -499.407, 310.001, 34.1184, 0, 14)
    table.insert(tblRoomTrophies, { id = idObject, bPool = bObjectPool })
    totalTrophies = totalTrophies + 1
  end
  ...
  if shared.gSkippedWeedKiller or StatGetAsInt(264) == 19 then
    ...
    idObject, bObjectPool =
      CreatePersistentEntity('Mission_Tomb', -497.552, 307.855, 31.8739, 0, 14)
    table.insert(tblRoomTrophies, { id = idObject, bPool = bObjectPool })
    totalTrophies = totalTrophies + 1
  end

  StatSetAsInt(1, totalTrophies)
end

function F_DeleteRoomTrophies()
  for _, Entry in tblRoomTrophies do
...
function F_CreateInteractiveSigns()
  ...
  local objID, objPool =
    CreatePersistentEntity(idSignObject, -499.849, 309.211, 33.0609, -180, 14)
  ...
  objID, objPool =
    CreatePersistentEntity(idSignObject, -495.223, 325.33, 33.0609, -180, 14)
  ...
  while not (AreaGetVisible() ~= 14 or SystemShouldEndScript()) do
    if FireAlarmFlag == true then
      ...
      while shared.gBDormFAlarmOn == true do
        if AreaGetVisible() ~= 14 then break end
        if SystemShouldEndScript() then break end
        Wait(0)
      end
...
function main()
  ...
  if IsMissionCompleated('4_B1') then DATLoad('BDorm_Spud.DAT', 0) end
  if IsMissionCompleated('3_R09_N') then DATLoad('BDorm_RLauncher.DAT', 0) end
  if IsMissionCompleated('2_03') then DATLoad('BDorm_Eggs.DAT', 0) end
  F_GaryOnBedInit()
  F_PreDATInit()
  DATInit()
  ...
  F_SetupDormSpawners()
  SetupBathroomProps() -- 2nd Floor Bathroom
  ...
  while not (AreaGetVisible() ~= 14 or SystemShouldEndScript()) do
    ...
    if
      not MissionActive()
      and not IsMissionCompleated('1_E01')
      and not PlayerIsInTrigger(TRIGGER._PLAYER_ROOM)
      and not bLaunchErrandTut
      and IsMissionCompleated('1_05')
      and ClockGet() >= 8
      and ClockGet() <= 18
    then
    ...
    if
      IsMissionCompleated('1_02A')
      and not gPlayerImmortal
      and not IsMissionCompleated('1_02B_Dummy')
    then
    ...
    if gBedWake == true then
      ...
      ExecuteActionNode(
        gPlayer,
        '/Global/Ambient/MissionSpec/WakeUp/GetUp',
    ...
    if
      (gClothingUnlocked or shared.unlockedClothing)
      and not shared.lockClothingManager
    then
      if not cx then
      ...
      if
        not gClothing
        and F_CheckPedNotInGrapple(gPlayer)
        and PlayerIsInAreaXYZ(cx, cy, cz, 1, 6, 90)
      then
  ...
  if gPlayerImmortal then PedSetFlag(gPlayer, 58, false) end
  F_CleanChemEffects()
  AreaClearDockers()
  AreaClearSpawners()
...
function F_SetupDormSpawners()
  ...
  -- ------------------------------------------------------------------------ --
  -- DOWNSTAIRS: default Rockstar setup                                       --
  -- ------------------------------------------------------------------------ --
  ...
  -- ------------------------------------------------------------------------ --
  -- UPSTAIRS: new triggers + points                                          --
  -- ------------------------------------------------------------------------ --

  table.insert(gBDormDoors, {
    Trigger = TRIGGER.BdrDoorUpstairs1,
    Point = POINTLIST.BdrDoorUpstairs1,
  })
  table.insert(gBDormDoors, {
    Trigger = TRIGGER.BdrDoorUpstairs2,
    Point = POINTLIST.BdrDoorUpstairs2,
  })
  table.insert(gBDormDoors, {
    Trigger = TRIGGER.BdrDoorUpstairs3,
    Point = POINTLIST.BdrDoorUpstairs3,
  })
  table.insert(gBDormDoors, {
    Trigger = TRIGGER.BdrDoorUpstairs4,
    Point = POINTLIST.BdrDoorUpstairs4,
  })
  table.insert(gBDormDoors, {
    Trigger = TRIGGER.BdrDoorUpstairs5,
    Point = POINTLIST.BdrDoorUpstairs5,
  })

  -- ------------------------------------------------------------------------ --
  -- Apply all spawn & dock location                                          --
  -- ------------------------------------------------------------------------ --
  ...
  AreaAddSpawnLocation(
    BEntranceDoor,
    POINTLIST.DormExitDoorR,
    TRIGGER.DormExitDoorR
  )
  AreaAddDockLocation(
    BExitDoor,
    POINTLIST.DormExitDoorL,
    TRIGGER.DT_DormExitDoorL
  )
  ...
  -- Spawn schedule (using in-game time)
...
-- Sets up invisible bathroom props for interaction (sink, toilet, urinal)
function SetupBathroomProps()
  local area = AreaGetVisible()
  local CPE = CreatePersistentEntity

  local rotation = 180
  CPE('pxSink', -514.996, 329.97, 38.1, rotation, area)
  CPE('pxSink', -513.875, 329.97, 38.1, rotation, area)
  CPE('pxSink', -512.652, 329.97, 38.1, rotation, area)
  CPE('pxSink', -511.399, 329.97, 38.1, rotation, area)

  CPE('pxToilet', -517, 329.65, 38.12, rotation, area)
  CPE('pxToilet', -519.2, 329.65, 38.12, rotation, area)

  rotation = 0
  CPE('pxUrnal', -514.03, 321.6, 38.12, rotation, area)
  CPE('pxUrnal', -515.02, 321.6, 38.12, rotation, area)
  CPE('pxUrnal', -515.96, 321.6, 38.12, rotation, area)
  CPE('pxUrnal', -516.91, 321.6, 38.12, rotation, area)
  CPE('pxUrnal', -517.78, 321.6, 38.12, rotation, area)
  CPE('pxUrnal', -518.78, 321.6, 38.12, rotation, area)

  rotation = -90
  CPE('pxWtrFtn', -520.53, 323.23, 38.12, rotation, area)
end

function UpdateAmbientSpawners()
  if FireAlarmFlag ~= false then return end
...
function F_ChemistrySet()
  ...
  elseif
    currentDay ~= shared.ChemistrySetLastDayUsed
    and shared.ChemistrySetLocked == 0
  then
    shared.ChemistrySetLocked = 1
  end
...
function F_CleanChemEffects()
  for _, lEffect in gChemEffects do
    if EffectIsRunning(lEffect) then EffectKill(lEffect) end
  end
end
...
function F_DeathKOTest()
  ...
  if
    PedIsInAreaXYZ(gPlayer, x, y, z, 0.25, 0)
    and PedGetFlag(gPlayer, 31) == true
  then
    gBedWake = true
  end
end
...
function F_DormTriggas(TriggerID, PedID)
  ...
  --print(TriggerID)
  if
    TriggerID == TRIGGER._DORMDOORTRIG1
    or TriggerID == TRIGGER._DORMDOORTRIG2
    or TriggerID == TRIGGER._DORMDOORTRIG3
    or TriggerID == TRIGGER._DORMDOORTRIG4
  then
    BMovePlaya = true
  end
end
...
function F_LoadGaryInBed()
  ...
  if not shared.b1x11_failed then LoadActionTree('Act/Conv/1_11X1.act') end
...
function F_UnloadGaryInBed()
  if PedIsValid(pedGary.id) then PedDelete(pedGary.id) end
end
...
function F_GarySpeaks()
  if PedIsValid(pedGary.id) then
    ...
    elseif
      PlayerIsInTrigger(TRIGGER._PLAYER_ROOM)
      and timerGarySpeak + 10000 <= GetTimer()
    then
      SoundPlayScriptedSpeechEventWrapper(
        pedGary.id,
        'WHERES_YOUR_COSTUME',
...
function F_KillAllLittleKids()
  PedSetUniqueModelStatus(69, -1)
  PedSetUniqueModelStatus(66, -1)
  if shared.gHalloweenActive == true then PedSetUniqueModelStatus(159, -1) end
end
...
function F_BringBackTheLittleOnes()
  ...
  if shared.gHalloweenActive == true then PedSetUniqueModelStatus(159, 2) end
end
